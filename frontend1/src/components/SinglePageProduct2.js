...
const SinglePageProduct2 = () => {
  ...
  return (
   <div className='bg-gray-200'>
    <Header/>
    <Navbar/>
    <div className='md:hidden initial:block'>
        <div className='flex justify-center'>
            <div className='w-1/2 h-[200px]'><img src={item.thumbnail} alt="loading" className='w-[200px] h-[200px] rounded-sm'/></div>
            <div className = 'w-1/2 h-[200px] bg-white flex justify-center'>
                        <div>
                        <div className='mt-[25px]'>
                <div className='text-3xl font-bold text-center'>₹{((item.price)*81).toLocaleString()}.00</div>
                <div className='line-through font-light text-center'>₹{(((item.price)*81)/(1-item.discountPercentage/100)).toFixed(2).toLocaleString()}</div>
                <div className='font-semibold text-lg text-center'>{item.discountPercentage}%off</div>
                <div className='font-semibold text-lg text-center '>⭐{item.rating}</div>
            </div>
                        </div>
                </div>
        </div>
        <div className='w-[full] h-[50px] bg-red-400 flex'>
            <div className='w-1/2 h-[50px] flex justify-center items-center bg-amber-500'><button onClick={()=>{
                handleAddToCart(item)
                handleToastClick()
                alert('item added to the cart.')
                }}>Add to Cart</button></div>
            <div className='w-1/2 h-[50px] flex justify-center items-center bg-green-500'> <Link to={'/buy?v='+item.id}><button>Buy</button></Link></div>
        </div>
    </div>
    <div className='bg-gray-200  md:block initial:hidden h-[500px] font-Poppins'>
        <div className="flex justify-center">
